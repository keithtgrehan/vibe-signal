...
def test_parser_handles_whatsapp_sample_text() -> None:
    sample = "[01.06.26, 09:00:00] self: Can you confirm Friday?\n[01.06.26, 09:01:00] other: Yes, Friday works."
...
def test_ingest_refuses_non_restricted_output_directory(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(zip_path, "w") as archive:
        archive.writestr("_chat.txt", "[01.06.26, 09:00:00] self: Can you confirm Friday?")
...
def test_scripts_do_not_print_raw_message_text(tmp_path: Path, capsys) -> None:
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "w") as archive:
            archive.writestr("_chat.txt", f"[01.06.26, 09:00:00] self: {raw_text}")
